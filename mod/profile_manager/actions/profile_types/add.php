	...
	*/
	...
	if(!empty($name)){
		if(!empty($guid)){
			...
			if(!empty($object) && $object->getSubtype() != CUSTOM_PROFILE_FIELDS_PROFILE_TYPE_SUBTYPE){
				$object = null;
			}
		}
		...
		if(empty($object)){
			...
			$object->subtype = CUSTOM_PROFILE_FIELDS_PROFILE_TYPE_SUBTYPE;
		...
		if(!empty($object)){
			...
			if(!empty($label)){
				...
				$object->clearMetadata("metadata_label");
			}
			
			if(!empty($description)){
				$object->metadata_description = $description;
			} else {
				$object->clearMetadata("metadata_description");
			}
