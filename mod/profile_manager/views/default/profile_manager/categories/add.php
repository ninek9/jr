	...
	if($type_count > 0){
		...
		foreach($types as $type){
			if(!empty($type->metadata_label)){
				$label = $type->metadata_label;
			} else {
				$trans_label = "profile_manager:profile_type:name:" . $type->metadata_name;
				if($trans_label != elgg_echo($trans_label)){
					$label = elgg_echo($trans_label);
				} else {
					$label = $type->metadata_name;
				}
			}
			
			$options[$label] = $type->guid;
		}
