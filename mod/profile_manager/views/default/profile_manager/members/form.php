<?php 
	...
	if($profile_type_count > 0){
		...
		foreach($profile_types as $profile_type){
			// label
			if(!empty($profile_type->metadata_label)){
				$title = $profile_type->metadata_label;
			} else {
				$translation_label = "profile:types:" . $profile_type->metadata_name;
				if($translation_label == elgg_echo($translation_label)){
					$title = $profile_type->metadata_name;
				} else {
					$title = elgg_echo("profile:types:" . $profile_type->metadata_name);
				}
			}
