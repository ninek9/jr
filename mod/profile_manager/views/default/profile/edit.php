...
<?php
	...
	if(!empty($cats)){
			...
			if($types_count > 0){
				...
				foreach($types as $type){
					...
					if(!empty($description)){
						...
						$types_description = "<span id=\"profile_type_desc\">" . $description . "</span>";
				...
				<script type="text/javascript">
					...
					function changeProfileType(){
						...
						$('#elgg_horizontal_tabbed_nav li:first>a').click();
						<?php } ?>
					}
				</script>
				<?php 
				echo "<p id=\"profile_type\">\n";
				echo "<label>\n";
				echo elgg_echo("profile_manager:profile:edit:custom_profile_type:label");
				echo elgg_view("input/pulldown", array("internalname" => "custom_profile_type", "options_values" => $options, "js" => "onchange='changeProfileType();'", "value" => $vars["entity"]->custom_profile_type));
				echo "</label>\n";
		...
		foreach($cats as $cat_guid => $cat){
			...
			// need to set up a counter so that we can add space between the field columns
			$counter = 1;
			...
			foreach($fields[$cat_guid] as $field){
				...
				if(!empty($field->metadata_label)){
					...
					$trans_key = "profile:" . $metadata_name;
					if($trans_key != elgg_echo($trans_key)){
						$title = elgg_echo($trans_key);
					} else {
						$title = $metadata_name;
